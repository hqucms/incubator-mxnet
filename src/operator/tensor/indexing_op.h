...
inline bool GatherNNShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(out_attrs->size(), 1U);
  // The shape of indices
  const mxnet::TShape& dshape = (*in_attrs)[0]; // (N, C, P)
  const mxnet::TShape& ishape = (*in_attrs)[1]; // (N, P, K)
  ...
  mxnet::TShape oshape({dshape[0], dshape[1], dshape[2], ishape[2]}); // (N, C, P, K)
...
inline bool GatherNNBackwardShape(const nnvm::NodeAttrs& attrs,
  ...
  const mxnet::TShape& dshape = (*in_attrs)[0];  // (N, C, P, K)
  const mxnet::TShape& ishape = (*in_attrs)[1];  // (N, P, K)
  const mxnet::TShape& oshape = (*out_attrs)[0]; // (N, C, P)
